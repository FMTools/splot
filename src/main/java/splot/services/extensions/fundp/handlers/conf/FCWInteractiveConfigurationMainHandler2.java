...
public abstract class FCWInteractiveConfigurationMainHandler2 extends
		FreeMarkerHandler {

	protected FCWInteractiveConfigurationElementsProducer confElementProducer = null;

	public FCWInteractiveConfigurationMainHandler2(String handlerName,
			HttpServlet servlet, Configuration configuration, Template template) {
		super(handlerName, servlet, configuration, template);
	}

	protected abstract ConfigurationEngine createConfigurationEngine(
			String modelLocatorString) throws HandlerExecutionException;
	...
	public void buildModel(HttpServletRequest request,
		...
		try {
			...
			String viewDir = getServlet().getServletContext().getRealPath("/")
					+ "extensions/views/"; // getServlet().getInitParameter("viewFilesPath");
			...
			if (!(requestQueryString.indexOf("newSession") == -1)) {
				...
				if (!((newSession == null)) && (!(newSession == ""))
						&& (newSession.compareToIgnoreCase("true") == 0)) {
					newSession = "true";
				} else {
					newSession = "false";
			...
			if (!(requestQueryString.indexOf("workflowExistence") == -1)) {
				workflowExistence = (String) request
						.getParameter("workflowExistence");
				if ((workflowExistence == null) || (workflowExistence == "")
						|| (workflowExistence.compareToIgnoreCase("true") != 0)) {
				...
				if (newSession.compareToIgnoreCase("true") == 0) {
					if (requestQueryString.indexOf("workflowName") == -1) {
						...
						throw new HandlerExecutionException(
								"Problem finding the workflow name");
					} else {
						workflow = (String) request
								.getParameter("workflowName");
						if ((workflow == "") || (workflow == null)
								|| (workflow.compareToIgnoreCase("false") == 0)) {
							workflow = "false";
							throw new HandlerExecutionException(
									"Problem finding the workflow name");
			...
			if (workflowExistence.compareToIgnoreCase("true") == 0) {
				if (newSession.compareToIgnoreCase("true") == 0) {
					if (requestQueryString.indexOf("featureModelName") == -1) {
						...
						throw new HandlerExecutionException(
								"Problem finding the feature model name");
					} else {
						featureModelName = (String) request
								.getParameter("featureModelName");
					...
					if (requestQueryString.indexOf("featureModelName") == -1) {
						featureModelName = (String) session
								.getAttribute("featureModelName");
						if ((featureModelName == "")
								|| (featureModelName == null)
								|| (featureModelName
										.compareToIgnoreCase("false") == 0)) {
			...
			if (workflowExistence.compareToIgnoreCase("true") == 0) {
				if (newSession.compareToIgnoreCase("true") == 0) {
					if (featureModelName.compareToIgnoreCase("false") == 0) {
						...
						featureModelFileName = Methods.getfeatureModelFileName(
								modelDir, featureModelName);
						if ((featureModelFileName == null)
							...
							featureModelFileName = "false";
							throw new HandlerExecutionException(
									"Problem finding the feature model file name");
						}

					}

				} else {
					featureModelFileName = (String) session
							.getAttribute("selectedModels");
					if ((featureModelFileName == null)
							|| (featureModelFileName == "")
							|| (featureModelFileName
									.compareToIgnoreCase("false") == 0)) {
				...
				if (requestQueryString.indexOf("selectedModels") != -1) {
					featureModelFileName = (String) request
							.getParameter("selectedModels");
					if ((featureModelFileName == null)
							|| (featureModelFileName == "")
							|| (featureModelFileName
									.compareToIgnoreCase("false") == 0)) {
			...
			ConfigurationEngine confEngine = (ConfigurationEngine) session
					.getAttribute("conf_engine");
			...
			if (op.compareToIgnoreCase("reset") == 0) {
				if (confEngine == null) {
					confEngine = createConfigurationEngine(getResourcePath()
							+ featureModelFileName);
					confEngine.reset();
			...
			if ((featureModelName == null) || (featureModelName == "")
					|| (featureModelName.compareToIgnoreCase("false") == 0)) {
				...
				if (newSession.compareToIgnoreCase("true") == 0) {
					...
					if (requestQueryString.indexOf("viewName") == -1) {
						...
						if ((workflow.compareToIgnoreCase("false") == 0)
							...
							viewName = "none";
							throw new HandlerExecutionException(
									"Problem finding the view name");

						} else {
							String tmpViewName = Methods.getTaskViewName(
									viewDir, workflow, featureModelName, task);
			...
			if (workflowExistence.compareToIgnoreCase("true") == 0) {
				if (newSession.compareToIgnoreCase("true") == 0) {
					if (requestQueryString.indexOf("userKey") == -1) {
						userKey = "false";
						throw new HandlerExecutionException(
								"Problem finding the user key");

					} else {
						userKey = (String) request.getParameter("userKey");
						if ((userKey == null) || (userKey == "")
								|| (userKey.compareToIgnoreCase("false") == 0)) {
							userKey = "false";
							throw new HandlerExecutionException(
									"Problem finding the user key");
						}
			...
			if ((newSession.compareToIgnoreCase("true") == 0)
				...
				uncoveredFeatures = Methods
						.getFeatureModelUncoveredFeaturesInAllocatedViews(
								featureModelName, viewDir, modelDir);
			}
			...
			configurationFileName = Methods.getConfiguredFileName(
					configuredModelPath, userKey);
			...
			if (confElementProducer == null) {
				confElementProducer = new FCWInteractiveConfigurationElementsProducer(
				...
				if (op.compareToIgnoreCase("reset") == 0) {
					if (newConfiguration.compareToIgnoreCase("true") == 0) {
						...
						for (ConfigurationStep step : confEngine.getSteps()) {
							...
							String stepElementData = confElementProducer
									.produceStepElement(step, stepData);
							stepData.put("configurationStepElement",
									stepElementData);
						...
						getFeatureModelChilds(featureModel.getRoot(), fmChilds,
								...
								confEngine.getModel().getName(), viewName,
								viewType);

						for (FeatureTreeNode feature : confEngine.getModel()
								.getNodes(confEngine.getModel().getRoot())) {
							Map featureData = new HashMap();

							FeatureInViewCheckingResult featureInViewCheckingResult = new FeatureInViewCheckingResult();
							String featureElementData = confElementProducer
									.produceFeatureElement(feature,
											featureData,
											getFeatureTemplateFile(), viewDir,
											modelDir, featureModelFileName,
											confEngine.getModel().getName(),
											viewName,
											featureInViewCheckingResult,
											viewType, fmChilds, userKey);
							featureElementData = featureElementData.replaceAll(
									"[\r][\n]", "");
							featureData.put("configurationFeatureElement",
									featureElementData);
						...
						confEngine = (ConfigurationEngine) session
								.getAttribute("conf_engine");
						...
						for (FeatureTreeNode feature : confEngine.getModel()
							...
							decisionResult = Methods.getFeatureDecisionInfo(
									modelDir, userKey, feature.getID());

							if (decisionResult.found) {
								stepsToUpdateList1.add(confEngine.configure(
										feature.getID(), decisionResult.value
												.equals("1") ? 1 : 0));
							}
						}
					...
					if (newConfiguration.compareToIgnoreCase("true") == 0) {
						...
						confEngine = (ConfigurationEngine) session
								.getAttribute("conf_engine");
						ConfigurationEngine tmpConfEngine = createConfigurationEngine(getResourcePath()
								+ featureModelFileName);

						tmpConfEngine.reset();
						tmpConfEngine.getSteps().remove(
								tmpConfEngine.getLastStep());
						...
						for (FeatureTreeNode featureNode : model.getNodes()) {
							if (featureNode.isInstantiated()) {
								stepsToUpdateList2.add(tmpConfEngine.configure(
										featureNode.getID(),
										featureNode.getValue()));
							}
						}
						...
						// TODO: Check and/or Remove dead code
...
//						}
				...
				if (op.compareToIgnoreCase("reset") == 0) {
					...
					getFeatureModelChilds(featureModel.getRoot(), fmChilds,
							viewDir, modelDir, featureModelFileName, confEngine
									.getModel().getName(), viewName, viewType);
					...
					for (FeatureTreeNode feature : confEngine.getModel()
						...
						FeatureInViewCheckingResult featureInViewCheckingResult = new FeatureInViewCheckingResult();
						String featureElementData = confElementProducer
								.produceFeatureElement(feature, featureData,
										getFeatureTemplateFile(), viewDir,
										modelDir, featureModelFileName,
										confEngine.getModel().getName(),
										viewName, featureInViewCheckingResult,
										viewType, fmChilds, userKey);
					...
					ConfigurationEngine tmpConfEngine = createConfigurationEngine(getResourcePath()
							+ featureModelFileName);
					tmpConfEngine.reset();
					tmpConfEngine.getSteps()
							.remove(tmpConfEngine.getLastStep());
					...
					// TODO: Check or remove dead code
...
//					}
			...
			templateModel.put("countFeatures", confEngine.getModel()
					.countFeatures());
			templateModel.put("countInstantiatedFeatures", confEngine
					.getModel().getInstantiatedNodes().size());
			templateModel.put("done", confEngine.isDone());
			templateModel
					.put("fm_views", Methods.getFeatureModelViews(viewDir));
			...
			e2.printStackTrace();
			throw new HandlerExecutionException(
					"Problems locating/acessing the feature model repository path",
					e2);
		}
	}
	...
	public static void getFeatureModelChilds(FeatureTreeNode featureTreeNode,
			LinkedList<FeatureTreeNode> featureList, String viewDir,
			String modelDir, String featureModelFileName,
			String featureModelName, String viewName, String visualizationType) {
		...
		try {
			...
			for (int i = 0; i < featureTreeNode.getChildCount(); i++) {
				...
				FeatureInViewCheckingResult featureInViewCheckingResult = new FeatureInViewCheckingResult();
				Methods.checkFeatureInViewStatus(
						(FeatureTreeNode) featureTreeNode.getChildAt(i),
						viewDir, modelDir, featureModelFileName,
						featureModelName, viewName,
						featureInViewCheckingResult, visualizationType);
				if (featureInViewCheckingResult.show) {
					featureList.add((FeatureTreeNode) featureTreeNode
							.getChildAt(i));
				}

				getFeatureModelChilds(
						(FeatureTreeNode) featureTreeNode.getChildAt(i),
						featureList, viewDir, modelDir, featureModelFileName,
						featureModelName, viewName, visualizationType);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
